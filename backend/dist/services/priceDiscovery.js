...
exports.priceDiscoveryService = void 0;
...
class PriceDiscoveryService {
    ...
    async getPriceDiscovery(category, location) {
    ...
    async calculatePriceDiscovery(category, location) {
        try {
            // Build query filter
            ...
            const historicalProducts = await Product_1.default.find({
                ...filter,
            ...
            const recentNegotiations = await Negotiation_1.default.find({
                productId: { $in: products.map(p => p._id) },
                status: 'completed',
                updatedAt: { $gte: thirtyDaysAgo }
            });
            // Calculate confidence based on data volume and recency
            const confidence = this.calculateConfidence(products.length, recentNegotiations.length, historicalProducts.length);
            return {
    ...
    calculateTrend(historicalProducts) {
        if (historicalProducts.length < 5)
            return 'stable';
    ...
    calculateConfidence(currentProducts, recentNegotiations, historicalDataPoints) {
    ...
    getDefaultPriceData(category, location) {
    ...
    async getPriceHistory(category, location, days = 30) {
        try {
            ...
            const products = await Product_1.default.find(filter).sort({ updatedAt: 1 });
            // Group by day
            const dailyData = {};
...
exports.priceDiscoveryService = new PriceDiscoveryService();
