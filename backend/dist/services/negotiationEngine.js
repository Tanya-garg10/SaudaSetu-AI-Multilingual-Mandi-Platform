...
const priceDiscovery_1 = require("./priceDiscovery");
class NegotiationEngine {
    async suggestCounterOffer(negotiationId, currentOfferPrice, currentOfferQuantity) {
        try {
            const negotiation = await Negotiation_1.default.findById(negotiationId)
                .populate('productId');
            if (!negotiation)
                return null;
            const product = negotiation.productId;
            const marketData = await priceDiscovery_1.priceDiscoveryService.getPriceDiscovery(product.category, `${product.location.city}, ${product.location.state}`);
    ...
    async analyzeFairness(negotiationId) {
        try {
            const negotiation = await Negotiation_1.default.findById(negotiationId)
            ...
            const buyerMessages = negotiation.messages.filter(m => m.senderId.toString() === negotiation.buyerId.toString()).length;
            const vendorMessages = negotiation.messages.filter(m => m.senderId.toString() === negotiation.vendorId.toString()).length;
    ...
    async getOptimalPrice(productId, quantity) {
        try {
...
exports.negotiationEngine = new NegotiationEngine();
